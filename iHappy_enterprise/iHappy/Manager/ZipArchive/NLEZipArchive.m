...
@interface NLEZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end


@implementation NLEZipArchive {
	NSString *_path;
	NSString *_filename;
    zipFile _zip;
}
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(id<NLEZipArchiveDelegate>)delegate {
	return [self unzipFileAtPath:path toDestination:destination overwrite:YES password:nil error:nil delegate:delegate];
}


+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<NLEZipArchiveDelegate>)delegate {
	// Begin opening
	...
	if (zip == NULL) {
		...
		if (error) {
			*error = [NSError errorWithDomain:@"NLEZipArchiveErrorDomain" code:-1 userInfo:userInfo];
		}
		return NO;
	}
	...
	if (unzGoToFirstFile(zip) != UNZ_OK) {
		...
		if (error) {
			*error = [NSError errorWithDomain:@"NLEZipArchiveErrorDomain" code:-2 userInfo:userInfo];
		}
		return NO;
	}
	...
	do {
        ...
        if (nil != err) {
            NSLog(@"[NLEZipArchive] Error: %@", err.localizedDescription);
        }
                ...
                if (fileInfo.dosDate != 0) {
                    ...
                    if (attr) {
                        if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[NLEZipArchive] Failed to set attributes");
                        }
                    }
                }
            }
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[d objectForKey:@"modDate"], NSFileModificationDate, nil] ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
            NSLog(@"[NLEZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
        }
        if (err) {
            NSLog(@"[NLEZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths {
	BOOL success = NO;
	NLEZipArchive *zipArchive = [[NLEZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
...
- (BOOL)close {    
	NSAssert((_zip != NULL), @"[NLEZipArchive] Attempting to close an archive which was never opened");
	zipClose(_zip, NULL);
	return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime {
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[NLEZipArchive] MSDOS date masks don't add up");
