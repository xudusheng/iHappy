...
- (void)refreshVisibleCellAppearance{
    ...
    if (_minimumPageAlpha == 1.0 && self.leftMargin == 0 && self.rightMargin == 0 && self.topMargin == 0 && self.bottomMargin == 0) {
        return;//无需更新
    }
    switch (self.orientation) {
        case HQFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                NSLog(@"offset = %lf ======== origin= %lf", offset, origin);
                ...
                if (delta < _pageSize.width) {
                    ...
                    CGFloat leftInset = self.leftMargin * delta / _pageSize.width;
                    CGFloat rightInset = self.rightMargin * delta / _pageSize.width;
                    CGFloat topInset = self.topMargin * delta / _pageSize.width;
                    CGFloat bottomInset = self.bottomMargin * delta / _pageSize.width;
                    
                    cell.layer.transform = CATransform3DMakeScale((_pageSize.width-leftInset - rightInset)/_pageSize.width,(_pageSize.height-topInset-bottomInset)/_pageSize.height, 1.0);
                    ...
                    cell.layer.transform = CATransform3DMakeScale((_pageSize.width-self.leftMargin-self.rightMargin)/_pageSize.width,(_pageSize.height-self.topMargin - self.bottomMargin)/_pageSize.height, 1.0);
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(self.topMargin, self.leftMargin, self.bottomMargin, self.rightMargin));
                    cell.mainImageView.frame = cell.bounds;
...
- (void)reloadData {
    ...
    if (_needsReload) {
        ...
        //重置pageWidth
        _pageSize = CGSizeMake(self.bounds.size.width - 2 * (self.leftMargin+self.rightMargin),(self.bounds.size.width - 2 * (self.leftMargin + self.rightMargin)) * 9 /16);
